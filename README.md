...
Within that, though, **every agent still gets its own Unix account and SSH key** — Claude gets one, Codex gets another, and so on — rather than everyone sharing a single identity. Since permissions come from group membership and not from anything tied to a specific account, this costs nothing extra to set up (see "Adding an agent" below) and buys two things a single shared account wouldn't: each agent's actions are individually attributable in the SSH and sudo logs, and revoking one agent's access (delete its key, or remove it from the group) doesn't touch any other agent's.

By default, that's identity for logging and revocation, not narrower permissions — every account in the group can request a redeploy of any target with any branch that target allows, since group membership alone is what the sudoers rule checks. `deploy-executor.sh`'s `ACCOUNT_BRANCH_PATTERNS` table is the opt-in way to narrow that per account: it's keyed by account name, and if the account that invoked sudo (`$SUDO_USER` — set reliably by sudo itself from the real calling identity, not something the caller's own environment can override) has an entry there, the requested branch must match *both* that entry's pattern *and* the target's own `ALLOWED_BRANCHES` — an intersection, so a per-account entry can only narrow what a target already allows, never widen it. An account with no entry is unrestricted beyond the target's own list, same as before this table existed, so adding a new agent (see "Adding an agent" below) never requires touching it unless you specifically want to scope that agent. Verified this in a sandbox: an account with `codex/*` configured was correctly rejected for a branch outside that pattern even though the target itself would otherwise allow it, and an account with no entry at all stayed fully unrestricted.

The gotcha worth knowing before you rely on this: because it's an intersection, a per-account pattern with *no overlap at all* against a target's `ALLOWED_BRANCHES` silently locks that account out of the target entirely, for every branch, not just the ones you meant to exclude. If you restrict an account to `claude/*` but the target's own list is `main`, `feature/x`, `codex/*`, that account can no longer deploy the target at all until `claude/*` (or something else it's allowed) is also added to the target's own `ALLOWED_BRANCHES`.
...
Only if you want *this* agent restricted to a subset of branches (not just what its own `config.yaml` politely sticks to, but something the host actually enforces): add an entry for its account name to `deploy-executor.sh`'s `ACCOUNT_BRANCH_PATTERNS` — see that table's own comments, and the gotcha about it intersecting with each target's `ALLOWED_BRANCHES` rather than adding to it, in "The threat model..." above.
